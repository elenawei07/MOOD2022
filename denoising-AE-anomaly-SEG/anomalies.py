...
def get_all_anomalies():
    return [draw_circle, draw_anomaly1, draw_anomaly2, draw_anomaly3, draw_anomaly4, draw_anomaly5,
            ...
            draw_anomaly12, draw_anomaly13, cut_paste]
...
def draw_circle(image, seed=0, radius=None):
    ...
    rr, cc = draw.ellipse(r, c, radius, radius)
...
def crop_and_paste_patch(image, patch_w, patch_h, transform, rotation=False):
    ...
    dot1, dot2 = np.where(img_arr > 0.05)
    start_point = random.choice(range(len(dot1)))
    patch_left = dot1[start_point]
    patch_top = dot2[start_point]
    ...
    labels[paste_top:paste_top+patch_h, paste_left:paste_left+patch_w] = 1.
    aug_image = image.copy()
    aug_image.paste(patch, (paste_left, paste_top), mask=mask)
    # aug_image.save('./fig_anomaly/anomaly/cutpaste_b.jpg')
    # labels_save = Image.fromarray(labels*255)
    # labels_save.save('./fig_anomaly/label/cutpaste_b.jpg', bit=1)
    aug_image = np.array(aug_image)
    
    return aug_image, labels


def cut_paste(image, area_ratio = (0.02, 0.15), aspect_ratio = ((0.3, 1) , (1, 3.3)), seed=0):
    ...
    cutpaste, labels = crop_and_paste_patch(image, patch_w, patch_h, transform, rotation = False)
    cutpaste = cutpaste / 255
    return cutpaste, labels
