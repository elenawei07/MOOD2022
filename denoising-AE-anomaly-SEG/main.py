...
def denoising(datapath: Path, identifier: str, data="brain", batch_size=8, lr=0.001, depth=3, wf=7):
    ...
    trainer.noise = noise
    trainer.train_dataloader, trainer.val_dataloader = get_denoising_dataloaders(datapath, batch_size, data)
    trainer.reset_state()
    return trainer


def noise_seg(datapath: Path, identifier=None, lr=0.001, batch_size=32, wf=7, depth=3, data="brain") -> Trainer:
    device = torch.device("cuda:0")
    ...
    trainer.train_dataloader, trainer.val_dataloader = get_segmentation_dataloaders(datapath, batch_size, data)
    trainer.reset_state()
    return trainer
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", type=str, default='/home/data/WJ/MOOD2022/data/train/abdom_train/abdom_train', help="path to directory with input data.")
    parser.add_argument("-o", "--output", type=str, default = '/home/data/WJ/MOOD2022/data/train/process_abdomen_train', help="path to directory to store preprocessed data.")
    parser.add_argument("--preprocess", type=bool, default=False, help="whether to run code to do data preprocessing (required once).")
    parser.add_argument("-d", "--data", type=str, default="abdomen", help="either 'brain' or 'abdomen'.")
    ...
    if method == "denoising":
        ...
        trainer = noise_seg(identifier=f"segmentation_{data}", data=data, wf=8, depth=3, lr=0.001, batch_size=32,
                            datapath=destination_dir)
        trainer.train(epoch_len=480, min_epochs=50, max_epochs=800, early_stopping=val_loss_early_stopping(30))
